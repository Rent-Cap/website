...
function rentHistoryToMonthlyRents(rentOn20190315, initialRent, tenancyStartDate, rentChanges, currentDate) {
    let monthlyRents = {};
    let startMonth;
    ...
    if (initialRent > 0) {
        ...
        throw new Error("Either tenancy start date must be provdied or rent on 2019-03-15");
    }
...
function calculateMaxRents(zip, monthlyRents) {
    let maxRents = {};
...
function findFirstIllegalIncreaseMonth(monthlyRents, maxRents) {
...
function calculateRefunds(monthlyRents, maxRents, currentDate) {
    // returns array of months entitled to a refund
    // [{month: "2019-03", maxRent: 140000}, ... ]
    // month is ISO Date month ("yyyy-MM"), rent is int of price in cents 
    let firstIllegalIncreaseMonth = findFirstIllegalIncreaseMonth(monthlyRents, maxRents);
...
function calculateRollback(monthlyRents, maxRents, currentDate) { // returns (int) cents of rent the user should be paying on current date // if the current rent is legal then it returns 0
...
function calculateRefund(overPayments, currentDate) {
    ...
    return Object.values(overPayments).reduce(function (a, b) { return a + b; });
}
...
function dateToISOMonth(d) {
    ...
    return dDate.getUTCFullYear() + "-" + ((dDate.getUTCMonth() + 1) + "").padStart(2, "0");
}
...
function cpiChangeLookup(zip, date) {
